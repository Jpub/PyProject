...
# QUESTIONS는 딕셔너리의 리스트이며,
# 각 딕셔너리는 함정이 있는 질문과 그에 대한 답변을 나타낸다.
# 딕셔너리는 키로 'question'(질문에 대한 텍스트),
# 'answer'(답에 대한 텍스트),
# 'accept'(플레이어의 답에 포함된 정답 문자열 목록)가 있다.
# (!) 여기에 추가할 여러분만의 함정이 있는 질문을 생각해 보자:
QUESTIONS = [
...
for questionNumber, qa in enumerate(QUESTIONS):  # 메인 프로그램 루프
    print('\n' * 40)  # 화면을 깨끗이 지운다.
